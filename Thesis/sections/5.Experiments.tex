...
In addition, we also calculate the \textit{Pearson correlation coefficient} \cite{everitt2002cambridge}. The values of the correlation range from $0.941282$ to $1$ for \texttt{TMIC}, and from $0.9446$ to $1$ for \texttt{TAC}. Figures \ref{fig:correlation-tmic} and \ref{fig:correlation-tac} show the correlation between the \textit{propagation factor} and the values of the metrics \texttt{TMIC} and \texttt{TAC} for the client library \textit{fr.inria.gforge.spoon:spoon-core:8.2.0} and the server library \textit{org.eclipse.platform:org.eclipse.osgi:3.16.0}.

\begin{figure}[ht!]
\begin{center}
\includegraphics[width=0.75\textwidth]{figures/results/Rplot_spoon-core_eclipse-osgi_TMIC.png}
\caption{Plot \textit{propagation factor} vs \texttt{TMIC}, with regression line. \textit{R} is the Pearson correlation coefficient, and \textit{p} corresponds to the confidence interval}
...
\caption{Plot \textit{propagation factor} vs \texttt{TAC}, with regression line. \textit{R} is the Pearson correlation coefficient, and \textit{p} corresponds to the confidence interval}
...
\caption{Plot of the sumation of the coupling measured at each distance (\texttt{DTMIC}) vs the covariance of \textit{propagation factor} and \texttt{TMIC}. \textit{p} corresponds to the confidence interval}
...
\caption{Plot of the sumation of the coupling measured at each distance (\texttt{DTAC}) vs the covariance of \textit{propagation factor} and \texttt{TAC}. \textit{p} corresponds to the confidence interval}
...
\caption{Plot of the maximum distance at which coupling is measured vs the correlation of \textit{propagation factor} and \texttt{TMIC}. \textit{p} corresponds to the confidence interval}
...
\caption{Plot of the maximum distance at which coupling is measured vs the correlation of \textit{propagation factor} and \texttt{TAC}. \textit{p} corresponds to the confidence interval}
