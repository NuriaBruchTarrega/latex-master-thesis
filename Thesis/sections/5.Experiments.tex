...
To evaluate the results of this experiment, we will discuss the meaning of the different cases as for detecting unused dependencies. There are 9 cases in which the unused dependencies were not correctly detected. Out of these 9, 4 can completely be fixed, if the tool could also analyze the tests defined in the client library. This could be fixed by doing source-code analysis including the testing, instead of bytecode analysis. In addition, the 2 cases in which the dependencies \todo{check why the parent dependencies are not included}. Finally, there are 3 cases in which there is at least one dependency which according to the paper should be unused, and it was detected used by the PoC. Therefore, the PoC implementation is overestimating in certain measure the usage of the dependencies.
...
Based on the results shown in Table \ref{table:summary-significance}, we can see that the combination of the two types of coupling metrics allow to detect used dependencies in 664/699 of the cases. In addition, the metrics \texttt{MIC} and \texttt{TMIC} alone, detect 659/699 of the cases. Which means that \texttt{AC} and \texttt{TAC} are only really needed in 5 of these dependencies. However, this only talks about whether or not the metrics can be used to perform a binary evaluation of the dependencies, which was not the goal of this thesis. Therefore, \texttt{AC} and \texttt{TAC} are also necessary in order to consider the type of coupling measured by this metric.
...
Nevertheless, since there are cases in which the combination of both types of metrics was not enough, it indicates that there are some other types of connections to be considered and measured. Given that half of the server libraries which were used but not detected by the coupling metrics contain annotations, creating a metric to measure the coupling created by annotations would improve this results.
...
For question 18, the interviewees answered with which metrics they considered more useful. Their answers can be found in Table \ref{table:interview-18}. As in question 12, a $\oplus$ mean that the interviewee mentioned the metric as one of the most useful, $\odot$ is for the metrics mentioned in a second place. And finally, if the cell is empty means that the metric was not mentioned.
...
\subsection{Discussion}
In this section, we discuss the answers to the interviews, divided in sections. First, there is a discussion on the general evaluation given by the interviewees, according to their role.

We have found, that there is a difference in the way the interviewees evaluate the model and the visualizations according to whether the main focus of the interviewee's rols is development, or arquitecture and analysis.

The developers want more informations that can be directly transformed into development actions. For example, the suggestions regarding the list of methods used of the dependencies, which can answer a vulnerable method is used or not. Also, the lines of code where the calls to a dependency are made, then they can directly go to that line of code to make the necessary changes.

However, the other type of interviewee need more high-level information. One of the suggestions made by several of these interviewees was to create a metric regarding how much of the client library depends on the server library. Also, how widespread the usage is in the client library. These two metrics would be related to the arquitecture of the system.

Furthermore, there is also an indicator of this difference in their answers to the prefered metric. The interviewees with tasks related to analysis and architecture prefered the metric \texttt{MIC}/\texttt{TMIC}, as they indicated in Question 18.

\subsubsection{Dependency Management}
With the questions answered in the \textit{Dependency Management} section we know that all the interviewees have experience with this type of tasks. Also, the interviewees considered more important to monitor the vulnerabilities of the dependencies, than update to the last version. This is also confirmed by the comments of some interviewees, saying that the main reason to update a dependency is to avoid being affected by possible vulnerabilities. \unsure{Should I add a finding about this? Even if it does not relate directly to any of the research questions}


\subsubsection{Visualizations}
When asked about the usefulness of the tool, there is a general agreement that the tool is useful in the scenarios discussed during the interview. There are some negative answers to whether the tool can make the dependency management easier. However, most of the comments about these answers are related to the tool being useful for very specific cases. Therefore, the negative answers are not related to the tool not making the tasks easier.
...
In the answers to Question 12, only 4 interviewees answer only one of the visualizations. Also, some of the other interviewees commented that there is value on the combination of the perspectives in the visualizations. Each visualization has a point of view which adds to the value of the entire tool. The \textit{Tree} visualization gives an understanding of the hierarchy of the dependencies, and where the transitive dependencies come from. The \textit{Table} visualization make it possible to compare the metrics, and therefore, the degree of dependency. Finally, the \textit{Barchart} gives a more detailed view of the impact of the dependencies in the client library.

\begin{finding}
	It is important to have different perspective on the dependencies of a system in the visualizations, to have a complete understanding of the dependency tree.
	\label{find:different-perspectives}
\end{finding}

With the last question about the visualizations, the interviewees suggested some improvements to be made. There are certain suggestions which are small improvements in terms of interaction with the visualizations, and some other additions to make it easier to use. According to the research method, the next step would be to implement some of these suggestions, and do more interviews. However, due to time limitations we cannot do these tasks.

In addition, some of the interviewees expressed a need to integrate the different sources of information about dependency management. This data includes vulnerability data, the new versions available of the dependencies, and the licenses of the libraries used. Some related suggestions into making the features of this tool into another type of tool are to make it a IDE Plugin, and a command interface. Hence, there is a general interest in the tool, but it might be important to consider changing the format in which these are supplied.

\unsure{Should these last two points be made into a finding?}

\subsubsection{Metrics}
Regarding the clarity and comprehensibility of the metrics, the average grade given by the interviewees for all the metrics is positive, being a 3.8/5 the lowest one, and a 4.73/5 the highest one.

\begin{finding}
	There is a general consensus that the metrics defined in this model are clear and comprehensible.
	\label{find:clear-comprehensible}
\end{finding}

The metrics with the lowest grade are the coupling metrics. As confirmed by the comments of the question about the actionability of the model, most of the interviewees agreed that the coupling metrics are harder to understand since there is not a clear scale. Therefore, a number gives some information on the dependency itself, as well as how it compares to the rest of dependencies in the tree, but there is no clear meaning which indicates if a certain value is "good" or "bad".
...
The answers to Question 16, about the usefulness of the model are mainly positive. Just as in the case of the visualizations, the interviewees giving the lowerst marks reasoned that the need to have such a detailed evaluation of the dependencies is not necessary on a daily basis.
...
With respect to the question 18, regarding which are the most useful metrics in the model. The metrics that got more votes are the coverage metrics, which is consistent with the answers to question 15, in which the coverage metrics have two of the highest average scores. Also, just as in the case of the visualizations, some of the interviewees commented that it is the different perspectives given by each of the metrics which makes the model more useful.

In the last question of the interviews, we gathered suggestions about the metrics in the model, can also be grouped into: changes to current metrics, and new metrics. One of the main suggestions obtained is to create a combination of the coupling metrics, to have a general evaluation, but also being able to dive in the detail with the current metrics. Then, the users would have first a general indicator, which can already point them to the dependencies that need a more detailed look.

\begin{finding}
	There is interest of a general metric indicating the general degree of dependency, as a combination of the existing metrics in the model.
	\label{find:general-metric}
\end{finding}

We have also obtained some suggestions which are modifications over the current metrics. For example, having the absolute number of reachable methods or classes, and the list of their names. It should be evaluated which is the need for this information, and in which cases it would be useful.

Finally, one of the suggestions to also improve the actionability of the coupling metrics is to create a risk profile of these metrics, by evaluating which are the common values of this metrics, and which are the outliers. These suggestion is further investigated in the following experiment.
