...
\paragraph{Property 2: Null value}
Following the description of the property, it is necessary to prove that: $R(L_c, L_s) = 0 \implies MIC(L_c, L_s) = 0$. Let $L_c$ be a client library and $L_s$ be a server library. For every $m_c \in M(L_c)$ and $m_s \in M(L_s)$, following the formal definition of the metric, we have that:
